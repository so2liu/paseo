...
export class AgentManager {
  ...
  private async initializeAgentTimelineForRegister(params: {
    ...
    if (
      ...
      !(await this.hasCommittedTimeline(agentId))
    ) {
      this.timelineStore.delete(agentId);
    }
  ...
  async hasCommittedTimeline(agentId: string): Promise<boolean> {
    ...
    return (await this.durableTimelineStore.getLatestCommittedSeq(agentId)) > 0;
  }
    ...
    if (!options?.fromHistory) {
      if (isTurnTerminalEvent(event)) {
        ...
        if (isForegroundEvent) {
          // `completed` keeps a finished turn from replaying queued input.
