...
export class Session {
  ...
  private dispatchAgentLifecycleMessage(msg: SessionInboundMessage): Promise<void> | undefined {
    ...
    const projectRequest = this.dispatchProjectRequest(msg);
  ...
  private dispatchProjectRequest(msg: SessionInboundMessage): Promise<void> | undefined {
    switch (msg.type) {
  ...
  private async handleFetchAgentTimelineRequest(
    ...
    try {
      // `source` is the requesting connection; the timeline itself comes from
      // whichever backing store can answer without starting a provider process.
      const timelineSource = await this.resolveTimelineSource(msg.agentId);

      const controlTimeline = await timelineSource.readTimeline({
        direction,
        cursor,
        limit: pageLimit,
      });
      const selectedTimeline = await this.selectTimelineProjection({
        readTimeline: timelineSource.readTimeline,
        projection,
        controlTimeline,
        direction,
        ...(cursor ? { cursor } : {}),
      ...
      this.emitForSource(
        {
          ...
          payload: {
            ...
            entries: selectedTimeline.entries.map((entry) => ({
              provider: timelineSource.provider,
              item: entry.item,
