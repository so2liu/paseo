...
// Upstream raised this from 2 minutes to 30 in v0.2.3, solving the same problem
// we did — but the owner wants idle agents warm for a full hour, so our longer
// TTL stays. The sweep interval was never ours; take upstream's.
const IDLE_AGENT_RUNTIME_TTL_MS = 60 * 60 * 1000;
...
export async function createPaseoDaemon(
  ...
  // Committed timeline rows outlive the agent runtime, so opening an old
  // conversation reads history off disk instead of resuming its provider
  // session first.
  const durableTimelineStore = new SqliteAgentTimelineStore({
    file: path.join(config.paseoHome, "timelines.db"),
    logger: logger.child({ component: "agent-timeline-store" }),
  });
  ...
  const stop = async () => {
    ...
    await agentManager.flushCommittedTimelines().catch(() => undefined);
    // Release the WAL and its sidecar files so the next start opens cleanly.
