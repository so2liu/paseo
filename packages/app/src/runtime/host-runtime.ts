...
export class HostRuntimeController {
  ...
  private async runProbeCycle(): Promise<void> {
    ...
    const maybeActivateFirstAvailable = async (
      ...
      const canActivateSuccessfulProbe = (): boolean =>
        !this.snapshot.activeConnectionId ||
        (!hasActiveOnlineConnection && this.snapshot.connectionStatus !== "online");

      while (canActivateSuccessfulProbe()) {
        if (!activationLock) {
          ...
          return (
            this.snapshot.activeConnectionId === connectionId && this.snapshot.client === client
          );
        }
        await activationLock;
      }
...
export class HostRuntimeStore {
  ...
  private async loadFromStorage(): Promise<void> {
    ...
    try {
      let stored = await readValidatedJson(
        this.storage,
        REGISTRY_STORAGE_KEY,
        StoredHostRegistrySchema,
      );
      // Chromium can lose its storage partition, taking the registry with it. The
      // desktop backup is the only copy left at that point.
      if (!stored && this.hostRegistryBackup) {
        try {
          const backup = await this.hostRegistryBackup.read();
          const parsed = backup ? StoredHostRegistrySchema.safeParse(JSON.parse(backup)) : null;
  ...
  private async persistHosts(hosts = this.hosts): Promise<void> {
    ...
    // The backup only mirrors what actually reached storage. Writing it after a failed
    // primary write would let a restore bring back a registry that was never persisted,
    // so the failure propagates and the backup is left alone.
