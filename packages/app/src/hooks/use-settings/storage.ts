...
// Upstream caps this at 21. Icon sizes and control heights scale with the UI ramp
// here, so the type is not the only thing that grows and the higher ceiling stays
// legible. The owner runs the app at this ceiling for eye strain.
export const MAX_UI_BASE_FONT_SIZE = 32;
...
const StoredAppSettingsSchema = z.strictObject({
  ...
  mobileComposerInputMode: z.enum(["voice", "text"]).optional(),
