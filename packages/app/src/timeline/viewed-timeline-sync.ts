...
export function createViewedTimelineSync(ports: ViewedTimelineSyncPorts): ViewedTimelineSync {
  ...
  return {
    ...
    setActive(nextActive) {
      ...
      // attention notifications use their separate delivery path. The hot set in
      // `recentlyViewedAgentIds` survives, so foregrounding resubscribes to it.
      if (!active && connected && deliveryMode === "selective") {
        commitDesiredMembership([]);
        return;
      }
      publishVisibleMembership();
    },
