...
const prBadgeStyles = StyleSheet.create((theme) => ({
  ...
  text: {
    ...
    lineHeight: Math.round(theme.fontSize.sm * 1.2),
    color: theme.colors.foregroundMuted,
  },
  textHovered: {
    color: theme.colors.foreground,
...
export function SidebarWorkspaceList({
  ...
  // Device mode groups by host, and the projection only knows the server id. The host's display
  // name lives here, so the label is resolved on the way into the list.
  const displayWorkspaceGroups = useMemo(
    () =>
      workspaceGroups.map((group) =>
        group.leading.kind === "device"
          ? {
              ...group,
              label: hostBadgeByServerId.get(group.leading.serverId)?.label ?? group.label,
            }
          : group,
      ),
    [hostBadgeByServerId, workspaceGroups],
  );
  ...
  const content =
    groupMode !== "project" ? (
      <SidebarGroupedModeList
        workspaceGroups={displayWorkspaceGroups}
