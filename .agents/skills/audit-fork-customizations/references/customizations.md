...
| MSG-006 | On desktop/web, Up at the start of the input recalls older user prompts; Down at the end moves newer and restores the draft. The visible input and selection must update.        | `1f2266bc6`, fork; regression guardrail | `composer/input-history.ts`; `composer/index.tsx` must call `replaceUserInput(result.text)`; require an integration test across the key handler and DOM-owned input |
