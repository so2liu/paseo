...
| TL-003 | The daemon uses upstream `FileAgentTimelineStore`; the retired SQLite runtime is read only to migrate complete caches without overwriting new data and is retained for rollback.                | upstream since v0.5.0 plus fork migration                    | `migrate-legacy-timeline-cache.ts`; migration unit and daemon upgrade E2E tests            |
