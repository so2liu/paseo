...
| MSG-006 | On desktop/web, Up at the start of the input recalls older user prompts; Down at the end moves newer and restores the draft. The visible input and selection must update from the live input snapshot without losing unpublished edits. | `1f2266bc6`, repaired in #59, fork | `composer/input-history.ts`; `composer/index.tsx` uses `event.input` and `replaceUserInput`; production composer Playwright test |
