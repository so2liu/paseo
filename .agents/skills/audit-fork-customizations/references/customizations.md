...
| MSG-004 | Active Claude, Codex, and Pi turns receive real steering through the current `steerActiveTurn`/`SteerResult` contract.                                                                                                                           | repaired in #60, fork/upstream API | provider `agent.ts` files; real Pi manager integration test; queue service and provider steering tests                                                          |
...
| MSG-006 | On desktop/web, Up at the start enters history, repeated Up/Down traverses it ahead of autocomplete, and Down past the newest entry restores the draft. Visible text and selection use the live input snapshot without losing unpublished edits. | `1f2266bc6`, repaired in #59, fork | `composer/input-history.ts`; `composer/index.tsx` uses live `event.input`, active navigation state, and `replaceUserInput`; production composer Playwright test |
...
| TL-002 | Committed history can be read without loading or resuming the agent, including complete histories cached by pre-v0.5 SQLite builds.                                                                                        | `949a616cd` lineage, fork on upstream store/migration        | `AgentManager.hasCommittedTimeline`; legacy migration and daemon timeline-cache read tests |
| TL-003 | The daemon uses upstream `FileAgentTimelineStore`; the retired SQLite runtime is read only to migrate complete caches without overwriting new data, and per-agent tombstones prevent Reload from resurrecting old history. | upstream since v0.5.0 plus fork migration                    | `migrate-legacy-timeline-cache.ts`; migration unit and daemon upgrade E2E tests            |
