...
be added here in the same PR that introduces it.

| Area                               | Fork behavior that must survive                                                                                                                                                                                                                                                                                                                     | Primary anchors                                                                                                                                                                                                     |
| ---------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Queued messages and steering       | New messages queue by default; queued state persists and syncs across clients; acknowledged messages remain visible; completed turns do not replay queued input; active Claude/Codex/Pi sessions receive real steer requests with provider-correct semantics, including Claude priority `now` and Pi's acknowledged native `steer` RPC.             | `packages/protocol/src/messages.ts`, `packages/server/src/server/agent/message-queue-service.ts`, provider `agent.ts` files, `packages/app/src/composer/message-queue.ts`                                           |
| Durable and fast timelines         | SQLite backs the daemon timeline cache; reconnect/resume loads the latest tail without duplicate answers or unchanged refresh churn; long conversations use collapsed logical assistant/execution groups and a native mobile-lite projection; the chat outline rail stays mounted at every panel width instead of hiding on narrow panels.          | `packages/server/src/server/agent/sqlite-agent-timeline-store.ts`, `packages/app/src/timeline/`, `packages/app/src/agent-stream/`, `packages/app/src/agent-stream/chat-outline/rail.web.tsx`, `docs/mobile-lite.md` |
...
| Review and unread semantics        | Review completion is explicit; displaying a workspace while the app is active marks only that attention generation as locally seen, so the green unread dot disappears, but it does not complete review and the durable status can remain `ready_to_view`; parent agents remain available while children work.                                      | `packages/app/src/stores/workspace-attention-view-store.ts`, sidebar workspace projections, `packages/protocol/src/agent-state-bucket.ts`, `docs/agent-lifecycle.md`                                                |
| Mobile readability and interaction | Large text scales line height and control geometry without clipping; cross-paragraph selection works on iOS; rich-copy keeps the complete conclusion; Markdown, code, diff, plan, and question surfaces avoid accidental taps and remain usable at large font sizes.                                                                                | `packages/app/src/components/message.tsx`, platform Markdown renderers, `packages/app/src/components/ui/control-geometry.ts`, rich clipboard utilities                                                              |
| File previews and downloads        | Mermaid and sandboxed HTML previews work on native and web, mount lazily, and keep Markdown/file scrolling responsive; native relay/socket/pipe downloads use the active encrypted binary transfer instead of assuming direct HTTP.                                                                                                                 | `packages/app/src/file-preview/`, `packages/app/src/stores/download-store.ts`, `packages/app/src/hooks/use-file-download.ts`, `docs/file-preview.md`                                                                |
...
| Agent reliability and models       | Idle agents remain resident indefinitely and are never collected merely for elapsed idle time (the upstream v0.2.5 behavior supersedes our former one-hour TTL); mixed-project push-token registration retries; Claude model probing preserves the catalog and correct context variants, including Opus 5; mobile thinking controls remain visible. | `docs/agent-lifecycle.md`, push service, Claude provider/catalog code, agent controls                                                                                                                               |
...
| Owner fleet deployment             | Every owner Mac upgrade is Desktop plus external daemon/CLI from one fork commit, with rollback and stable process ownership; iOS routine installs overwrite `Paseo Debug` using Xcode Release; Linux owner hosts receive the matching custom daemon build.                                                                                         | this section, `docs/development.md`, `docs/fork-releases.md`                                                                                                                                                        |
| Fork engineering policy            | Agents fix reported bugs through verified PRs instead of stopping at diagnosis; syncs use release tags, preserve merge ancestry, and keep customization evidence; Windows CI remains intentionally disabled until explicitly re-enabled.                                                                                                            | `CLAUDE.md`, `docs/upstream-sync.md`, `.github/workflows/ci.yml`                                                                                                                                                    |
