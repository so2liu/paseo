...
- Selective daemons receive every agent visible in any pane plus the most recently viewed hidden
  ...
  This fork narrows one case: app backgrounding drops the whole subscription set immediately so
  native clients stop receiving execution traffic, while the separate attention and push paths keep
  working. The hot set itself survives, so foregrounding resubscribes to it. See
  [mobile-lite.md](mobile-lite.md).
