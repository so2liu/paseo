...
append-only log per agent (see [data-model.md](./data-model.md#1a-committed-agent-timeline)), and
...
- A resumed agent seeds its timeline from the log and skips provider history replay. If the
  provider transcript was mutated outside Paseo, the log is what the user sees until they use
  **Reload agent**, which deletes the log and re-streams from the provider.
- A crash can lose rows that were still queued for append. The provider transcript remains the
  authority, so **Reload agent** recovers them.
...
- Durable committed rows: `packages/server/src/server/agent/jsonl-agent-timeline-store.ts`
